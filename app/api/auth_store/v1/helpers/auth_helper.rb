    ...
    module Helpers
      class AuthHelper
        ...
        def user_signup(params)
          if User.find_by(email_id: params[:emailid])
            ...
            if user.save
              ...
              {user_id: user.id, user_name: user.username, user_email: user.email_id, token: jwt_token, expires_at: expiry_date}
            else
        ...
        def user_logout(params)
          user = User.find_by(email_id: params[:emailid])
          if user
            {user_id: user.id, user_name: user.username, user_email: user.email_id, token: nil, expires_at: nil}
          else
